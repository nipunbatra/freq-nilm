...
class AppliancesRNN(nn.Module):
    ...
    def forward(self, *args):
        ...
        :param args: [aggregate_energy, p, appliance_1_energy,...IN self.order]
        :return:
        """
        agg_current = args[0]
        p = args[1]
        ...
        for appliance_num in range(self.num_appliance):
            ...
            if flag:
                ...
                agg_current = agg_current - args[2 + appliance_num]
...
def disagg_fold(fold_num, cell_type, hidden_size,
    ...
    for t in range(num_iterations):
        ...
        out = torch.cat([out_train[appliance] for appliance_num, appliance in enumerate(order)])
        if cuda_av:
            inp = inp.cuda()
            out = out.cuda()
...
pe = disagg_fold(fold_num, cell_type, hidden_size,
                num_layers, bidirectional, lr,
                num_iterations, order, p)
