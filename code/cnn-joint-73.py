...
class CustomCNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 16, kernel_size=(7, 3), stride=1, padding=2)
        ...
        self.conv6 = nn.ConvTranspose2d(16, 1, kernel_size=(7, 3), stride=1, padding=2)
...
def preprocess(train, valid, test):
    ...
    for a_num, appliance in enumerate(ORDER):
        out_train[a_num] = Variable(
            torch.Tensor(train[:, APPLIANCE_ORDER.index(appliance), :, :].reshape((train.shape[0], 1, train.shape[2], train.shape[3]))))
        if cuda_av:
    ...
    for a_num, appliance in enumerate(ORDER):
        out_valid[a_num] = Variable(
            torch.Tensor(valid[:, APPLIANCE_ORDER.index(appliance), :, :].reshape((valid.shape[0], 1, train.shape[2], train.shape[3]))))
        if cuda_av:
    ...
    for a_num, appliance in enumerate(ORDER):
        out_test[a_num] = Variable(
            torch.Tensor(test[:, APPLIANCE_ORDER.index(appliance), :, :].reshape((test.shape[0], 1, train.shape[2], train.shape[3]))))
        if cuda_av:
...
def disagg_fold(dataset, fold_num, lr, p):
    ...
    train_aggregate = train[:, 0, :, :].reshape(train.shape[0], 1, -1, train.shape[3])
    valid_aggregate = valid[:, 0, :, :].reshape(valid.shape[0], 1, -1, train.shape[3])
    test_aggregate = test[:, 0, :, :].reshape(test.shape[0], 1, -1, train.shape[3])
    ...
    for t in range(1000, num_iterations + 1, 1000):
        ...
        if cuda_av:
            for appliance_num, appliance in enumerate(ORDER):
                train_fold[t][appliance_num] = train_pred[t][appliance_num].cpu().data.numpy().reshape(-1, train.shape[3])
        else:
            for appliance_num, appliance in enumerate(ORDER):
                train_fold[t][appliance_num] = train_pred[t][appliance_num].data.numpy().reshape(-1, train.shape[3])
    ...
    for t in range(1000, num_iterations + 1, 1000):
        ...
        if cuda_av:
            for appliance_num, appliance in enumerate(ORDER):
                valid_fold[t][appliance_num] = valid_pred[t][appliance_num].cpu().data.numpy().reshape(-1, train.shape[3])
        else:
            for appliance_num, appliance in enumerate(ORDER):
                valid_fold[t][appliance_num] = valid_pred[t][appliance_num].data.numpy().reshape(-1, train.shape[3])
    ...
    for t in range(1000, num_iterations + 1, 1000):
        ...
        if cuda_av:
            for appliance_num, appliance in enumerate(ORDER):
                test_fold[t][appliance_num] = test_pred[t][appliance_num].cpu().data.numpy().reshape(-1, train.shape[3])
        else:
            for appliance_num, appliance in enumerate(ORDER):
                test_fold[t][appliance_num] = test_pred[t][appliance_num].data.numpy().reshape(-1, train.shape[3])
...
directory = "./baseline/cnn-joint/7_3/{}/{}/{}/{}/{}".format(dataset, fold_num, lr, num_iterations, p)
