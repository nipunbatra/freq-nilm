...
for learning_rate_cv in [0.1, 0.5, 1, 2]:
	...
	B_store[learning_rate_cv] = {}
	err_store[learning_rate_cv] = {}
	for num_season_factors_cv in range(2, 6):
		B_store[learning_rate_cv][num_season_factors_cv] = {}
		...
		for num_home_factors_cv in range(2, 6):
			...
			print("-" * 80)
			sys.stdout.flush()
			B_store[learning_rate_cv][num_season_factors_cv][num_home_factors_cv] = {}
			...
			H_source, B_source, A_source, T_source, Hs, Bs, As, Ts, HBATs, costs = learn_HBAT_adagrad_graph(source_tensor,
																											num_home_factors_cv,
			...
			for num_iterations in range(100, max_num_iterations, 200):
				A_store[learning_rate_cv][num_season_factors_cv][num_home_factors_cv][num_iterations] = As[num_iterations]
				B_store[learning_rate_cv][num_season_factors_cv][num_home_factors_cv][num_iterations] = Bs[num_iterations]
				print(learning_rate_cv, num_season_factors_cv, num_home_factors_cv, num_iterations, costs[num_iterations])
				sys.stdout.flush()
			...
			# 		pred[appliance] = []
			# 	for appliance in APPLIANCES_ORDER:
			# 		pred[appliance].append(pd.DataFrame(HBATs[num_iterations][:, appliance_index[appliance], :], index=source_df.index))
			# 	for appliance in APPLIANCES_ORDER:
			...
			# 	err_store[learning_rate_cv][num_season_factors_cv][num_home_factors_cv][num_iterations] = mean_err
																																
pickle.dump(A_store, open('../predictions/HBAS_{}_As.pkl'.format(source), 'w'))
pickle.dump(B_store, open('../predictions/HBAS_{}_Bs.pkl'.format(source), 'w'))
