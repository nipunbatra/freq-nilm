...
class CustomCNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 16, kernel_size=(7, 12), stride=1, padding=2)
        ...
        self.conv6 = nn.ConvTranspose2d(16, 1, kernel_size=(7, 12), stride=1, padding=2)
...
def preprocess(train, valid, test):
    ...
    for a_num, appliance in enumerate(ORDER):
        out_train[a_num] = Variable(
            torch.Tensor(train[:, APPLIANCE_ORDER.index(appliance), :, :].reshape((train.shape[0], 1, -1, train.shape[3]))))
        if cuda_av:
    ...
    for a_num, appliance in enumerate(ORDER):
        out_valid[a_num] = Variable(
            torch.Tensor(valid[:, APPLIANCE_ORDER.index(appliance), :, :].reshape((valid.shape[0], 1, -1, train.shape[3]))))
        if cuda_av:
    ...
    for a_num, appliance in enumerate(ORDER):
        out_test[a_num] = Variable(
            torch.Tensor(test[:, APPLIANCE_ORDER.index(appliance), :, :].reshape((test.shape[0], 1, -1, train.shape[3]))))
        if cuda_av:
...
directory = "./baseline/cnn-joint/7_12/{}/{}/{}/{}/{}".format(dataset, fold_num, lr, num_iterations, p)
