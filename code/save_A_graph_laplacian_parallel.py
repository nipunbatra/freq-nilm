...
def TF(case, source_tensor, source_L,max_num_iterations,learning_rate_cv, num_season_factors_cv, num_home_factors_cv, lam_cv):
	print max_num_iterations, learning_rate_cv, num_season_factors_cv, num_home_factors_cv, lam_cv
	H_source, A_source, T_source, Hs, As, Ts, HATs, costs = learn_HAT_adagrad_graph(case, source_tensor,
				                                                                                ...
				                                                                                lam=lam_cv,
				                                                                                T_known=T_constant)
	return H_source, A_source, T_source, Hs, As, Ts, HATs, costs
...
for learning_rate_cv in [0.1, 0.5, 1, 2]:
	...
	H_store[learning_rate_cv] = {}
	err_store[learning_rate_cv] = {}
	for num_season_factors_cv in range(2, 6):
		H_store[learning_rate_cv][num_season_factors_cv] = {}
		...
		for num_home_factors_cv in range(2, 6):
			if case == 4:
				if num_home_factors_cv != num_season_factors_cv:
					...
					continue
			H_store[learning_rate_cv][num_season_factors_cv][num_home_factors_cv] = {}
			...
			for lam_cv in [0.001, 0.01, 0.1, 0, 1]:
				H_store[learning_rate_cv][num_season_factors_cv][num_home_factors_cv][lam_cv] = {}
				...
				params[count] = []
				params[count].extend((case, source_tensor, source_L, 
									  max_num_iterations, 
									  learning_rate_cv, 
									  num_season_factors_cv, 
									  num_home_factors_cv,
...
for i in range(count):
	result = pool.apply_async(TF, params[i])
...
for i in range(count):
	case, source_tensor, source_L, max_num_iterations, learning_rate_cv, num_season_factors_cv, num_home_factors_cv, lam_cv = params[i]
	H_source, A_source, T_source, Hs, As, Ts, HATs, costs = r[i]
	for num_iterations in range(100, 1400, 200):
		...
		H_store[learning_rate_cv][num_season_factors_cv][num_home_factors_cv][lam_cv][num_iterations] = Hs[num_iterations]
